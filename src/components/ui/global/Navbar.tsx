...
<nav className="bg-white fixed w-full z-20 top-0 start-0 dark:border-gray-600">
  ...
  <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse items-center">
      <button className="bg-black text-white mr-3 flex items-center h-8 rounded-lg text-[10px] md:text-base px-6 text-center ">Get a Quote</button>
        ...
        <Cross toggle={setOpen} toggled={isOpen} size={22} />
            {navbarLinks.map((link, index) => (
              <li key={index} className="mb-2 last:mb-0">
                <Link href={link.href} className="block px-3 text-black dark:text-white text-left text-lg" aria-label={link.title}>
                  {link.title}
                </Link>
              </li>
            ))}
  ...
  */}
  ...
  <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
    <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
        {navbarLinks.map((link, index) => (
      <li key={index}>
        <Link href={link.href} className="py-2 px-3 text-black" aria-label={link.title}>{link.title}</Link>
      </li>
        ))}
    </ul>
  </div>
